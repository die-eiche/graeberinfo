...
//
//  Patched for modern iOS: URLSession instead of NSURLConnection.sendSynchronousRequest
...
class HBCIDDVConnection : HBCIConnection {
    ...
    init(host:String) throws {
        ...
        if let inpStr = inp, let outStr = out {
            ...
            logInfo("Unable to open connection to server \(host)");
